...
function [xy_coordinates, image_dilated] = process_tattoo_image(imgobj)
%% Process tattoo image
    % myTattoo = imread(filename);
    % myTattooColor = myTattoo;
    h1 = figure(3); imshow(imgobj); title('Original in Grayscale.');

    disp('Select a color for search and press enter.')

    [x,y] = getpts();

    target_color = imgobj(uint16(y),uint16(x),:);

    target_color = reshape(target_color, [1,1,3]);

    difference = single(imgobj) - single(target_color);
